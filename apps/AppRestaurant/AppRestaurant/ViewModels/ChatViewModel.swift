...
//  AppRestaurant
//
//  View model for agent chat using WebSocket (agents-whatsapp-rust protocol)
//
...
class ChatViewModel: ObservableObject {
    ...
    @Published var currentAgent: AgentRole = .host
    ...
    var baseURL: String {
        ...
        //   kubectl port-forward -n messaging svc/messaging-service 8080:80
    ...
    private func handleReceivedMessages(_ receivedMessages: [ReceivedMessage]) {
        ...
        for receivedMsg in receivedMessages {
            ...
            let chatMessage = ChatMessage(
                ...
                agentRole: receivedMsg.agentRole,
    ...
    func sendMessage(_ text: String) async {
        ...
        let clientMessageId = await wsService.sendTextMessage(
            content: text,
            conversationId: conversationId,
            receiverId: currentAgent.agentId,
        ...
        if let index = messages.firstIndex(where: { $0.id == userMessage.id }) {
            let updatedMessage = ChatMessage(
                ...
                agentRole: userMessage.agentRole,
    ...
    func switchAgent(_ role: AgentRole) {
        currentAgent = role
        // Create a new conversation ID for the new agent
        conversationId = UUID().uuidString
        messages = []
        // Load persisted messages for new conversation
        loadPersistedMessages()
    }
}
